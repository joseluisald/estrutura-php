...
{
	return API["url"] . '/assets/uploads/banners/'.$image;
}
...
{
	try {
		if ($_SERVER['SERVER_NAME'] == SITE_LOCAL && file_exists(dirname(__DIR__, 2) . "/src/$theme/scss/pages/" . $page . ".scss")) {
...
{
	if ($_SERVER['SERVER_NAME'] == SITE_LOCAL && file_exists(dirname(__DIR__, 2) . "/src/$theme/js/pages/" . $page . ".js")) {
		$compiler = new JS();
...
function cors()
{
	header('Set-Cookie: 5asecUC=true; SameSite=None;Secure');
